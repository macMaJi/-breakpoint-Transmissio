...
//此处我们可以从url中截取文件夹的名字 例如:"zhengfadaqin"、"征伐大秦.ipa"
#define  Filename  @"大掌门2.ipa"
// 文件的存放路径（caches）
...
-(BOOL)deleteTaskAndFile{
    NSError *error;
    BOOL isSuccess = [[NSFileManager defaultManager] removeItemAtPath:FileStorePath error:&error];
    if (isSuccess) {
        NSMutableDictionary *dict = [NSMutableDictionary dictionary];
        dict[Filename] = @0;
        [dict writeToFile: TotalLengthPlist atomically:YES];
        NSInteger count = [[[NSFileManager defaultManager] attributesOfItemAtPath: FileStorePath error:nil][NSFileSize] integerValue];
        // 关闭流
        ...
        NSLog(@"--删除成功--已下载大小%ld",(long)count);
    }else{
        NSLog(@"%@",error);
    }
    return isSuccess;
...
- (NSURLSessionDataTask *)task
{
    if (!_task) {
        ...
        BOOL isExist = [[NSFileManager defaultManager] fileExistsAtPath:FileStorePath];
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error
{
    ...
    }else if (DownloadLength != [[NSDictionary dictionaryWithContentsOfFile: TotalLengthPlist][ Filename] integerValue]){
        NSLog(@"下载出错!!!!");
        if (self.failedBlock) {
            self.failedBlock(error);
        }
